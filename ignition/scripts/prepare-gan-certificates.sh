...
GAN_CA_SECRETS_DIR="/run/secrets/gan-issuer-tls"
...
###############################################################################
# Places GAN CA Certificate and GAN Client Keystores
###############################################################################
function main() {
  # Populate GAN CA Certificate into the trusted certs folder
  populateGanCaCertificate

  # Update the alias in the inbound GAN PKCS#12 keystore and drop into place
  populateGanKeystore
...
###############################################################################
# Places the GAN Client Keystore
###############################################################################
function populateGanKeystore() {
  ...
  existing_alias=$(keytool -list -keystore "${keystore_path}" -storepass ${METRO_KEYSTORE_PASSPHRASE} | grep PrivateKeyEntry | cut -d, -f 1)
  target_alias="${METRO_KEYSTORE_ALIAS}"
  if [ "${existing_alias}" != "${target_alias}" ]; then
    info "Updating GAN Client Keystore alias from '${existing_alias}' to '${target_alias}'"
    keytool -changealias -alias "${existing_alias}" -destalias "${target_alias}" \
      -keystore "${keystore_path}" -storepass ${METRO_KEYSTORE_PASSPHRASE}
  fi
}
...
function updateGwbk() {
  ...
  for dest in "${dest_locations[@]}"; do
    zip -j "${GWBK_LOCATION}" "${GAN_CA_SECRETS_DIR}/tls.crt" "${dest}"
...
if [ ! -f "${GAN_CA_SECRETS_DIR}/tls.crt" ]; then
  >&2 echo "ERROR: GAN CA Certificate not found at ${GAN_CA_SECRETS_DIR}/tls.crt"
  usage
  exit 1
fi
